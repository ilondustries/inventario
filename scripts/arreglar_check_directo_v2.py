#!/usr/bin/env python3
"""
Script para arreglar la restricción CHECK directamente usando SQL (versión corregida)
"""
...
def arreglar_check_directo():
    ...
    try:
        ...
        # Reemplazar la restricción CHECK (manejar línea separada)
        print("🔄 Actualizando restricción CHECK...")
        
        # Buscar y reemplazar la restricción CHECK
        if "CHECK (estado IN ('pendiente', 'aprobado', 'rechazado', 'entregado'))" in current_sql:
        ...
        elif "CHECK(estado IN ('pendiente', 'aprobado', 'rechazado', 'entregado'))" in current_sql:
            new_sql = current_sql.replace(
                "CHECK(estado IN ('pendiente', 'aprobado', 'rechazado', 'entregado'))",
                "CHECK(estado IN ('pendiente', 'aprobado', 'rechazado', 'entregado', 'devuelto'))"
            )
            ...
            if "estado IN ('pendiente', 'aprobado', 'rechazado', 'entregado')" in current_sql:
                new_sql = current_sql.replace(
                    "estado IN ('pendiente', 'aprobado', 'rechazado', 'entregado')",
                    "estado IN ('pendiente', 'aprobado', 'rechazado', 'entregado', 'devuelto')"
                )
            else:
                print("❌ No se encontró ningún patrón de restricción CHECK")
                return False
